...
#define LCD_E_PIN          0x08 //0000 1000 //pin3
#define LCR_RW_PIN         0x10// 0001 0000 //pin4
#define LCD_RS_PIN         0x20// 0010 0000 //pin5
...
{	
 //Clock Gating Control
  RCGC_GPIO_R |= 0x12;     //enable clock for Port B & E
	...
	GPIO_PORTE_DIR_R |= 0x38;     // Port E pins as output 0011 1000
...
void light_sensor()
{
  RCGC_GPIO_R  |= 0x01; //turn on port f and portb
  
  //GPIOF->LOCK = 0x4C4F434B; //Enables write access to the GPIOCR register 
  GPIOA->DIR = 0x08; //1000, pin3 as output, pin2 as input
  GPIOA->DEN = 0x0C; //1100 pin 3 and pin2 

  GPIOA->PUR = 0x04; //0100 Enables weak pull-up resistors [pg.638]                   //switches and LEDs were enabled here.
  
	while(1)	{		
    switch(GPIOA->DATA & 0x04)  //0100
    {
    case 0x00:
        GPIOA->DATA = ~0x08;
        break;
...
RCGC_GPIO_R |= 0x1; //turn on port f and portb
  
  //GPIOF->LOCK = 0x4C4F434B; //Enables write access to the GPIOCR register 
  GPIOA->DIR = 0x20; //0100, pin 1 and pin 2 for leds
  GPIOA->DEN = 0x30; //0110 0110 GPIODEN is the digital enable register. Set pin = 1 to enable pin.

  GPIOA->PUR = 0x10; //0010 Enables weak pull-up resistors [pg.638]                   //switches and LEDs were enabled here.
  
  
				
    switch(GPIOA->DATA & 0x10)  //0010
    {
    case 0x00:
        GPIOA->DATA = 0x20;
        break;
